...
class MLP:
    ...
    def feed_forward(self, x):
        ...
        hidden_ = self.sigmoid(
            np.dot(input_, self.weights[0]))  # выход скрытого слоя = сигмоида(входные сигналы*веса скрытого слоя)
        output_ = self.sigmoid(np.dot(hidden_, self.weights[
            1]))  # выход сети (последнего слоя) = сигмоида(выход скрытого слоя*веса выходного слоя)
...
class MLPptorch(nn.Module):
    ...
    def __init__(self, in_size, hidden_size, out_size, active_function):
        ...
        layers.append(nn.Linear(in_size, hidden_size[0]))
        layers.append(active_function)  # Входной

        for i in range(1, len(hidden_size)):
            layers.append(nn.Linear(hidden_size[i -1], hidden_size[i]))
            layers.append(active_function)

        layers.append(nn.Linear(hidden_size[-1], out_size)) #выходной
        layers.append(active_function)
